...
int main(int argc, char *argv[]) {
	...
	BitmapType type = DetectBitmaps("");
	Bitmap* result;
	if(type == NO_BITMAPS) {
		exit(EXIT_FAILURE);
	}
	result = DecodeBitmap("",type,num,0,0);
	if(result == NULL) {
		exit(EXIT_FAILURE);
	}

	FILE *out;
	out = fopen(strcat(argv[1],".bmp"), "wb");
	unsigned char header[40] = "BM";
    int padding = ((bitmap->width)*3)%4;
    if (padding != 0) padding = 4 - padding;
	int datasize = ((bitmap->width)*3 + padding)*(bitmap->height);
	int offset = 14 + 40;
	int bmpsize = offset + datasize;

	for(x=2;x<14;x++) {
		header[x]=0;
	}
	header[2]=bmpsize&0xFF;
	header[3]=(bmpsize&0xFF00)>>8;
	header[4]=(bmpsize&0xFF0000)>>16;
	header[5]=(bmpsize&0xFF000000)>>24;
	header[10]=offset&0xFF;
	header[11]=(offset&0xFF00)>>8;
	header[12]=(offset&0xFF0000)>>16;
	header[13]=(offset&0xFF000000)>>24;
	...
	header[0]=40;
	header[4]=bitmap->width&0xFF;
	header[5]=(bitmap->width&0xFF00)>>8;
	header[8]=bitmap->height&0xFF;
	header[9]=(bitmap->height&0xFF00)>>8;
	header[12]=1;
	header[14]=24;
	header[20]=datasize&0xFF;
	header[21]=(datasize&0xFF00)>>8;
	header[22]=(datasize&0xFF0000)>>16;
	header[23]=(datasize&0xFF000000)>>24;
	header[24]=255;
	header[28]=255;
	fwrite(header, 40, 1, out);

    int zero = 0;
	for(y=bitmap->height-1;y>=0;y--) {
		for(x=0;x<bitmap->width;x++) {
			pixel = bitmap->palette[bitmap->bitmap[y*bitmap->width+x]];
			fwrite(&pixel.blue, 1, 1, out);
			fwrite(&pixel.green, 1, 1, out);
			fwrite(&pixel.red, 1, 1, out);
		}
        for(x=0;x<padding;x++) {
            fwrite(&zero, 1, 1, out);
        }
